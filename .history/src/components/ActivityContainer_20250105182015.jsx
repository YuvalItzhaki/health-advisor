...
const ActivityContainer = () => {
  ...
  useEffect(() => {
    ...
    const fetchSessions = async (startTimeMillis, endTimeMillis) => {
      const url = `https://www.googleapis.com/fitness/v1/users/me/sessions?startTime=<span class="math-inline">\{new Date\(startTimeMillis\)\.toISOString\(\)\}&endTime\=</span>{new Date(endTimeMillis).toISOString()}`;
      try {
        ...
        // Extract sessions and store datasetId along with other session data
        const sessions = (data.session || []).map((session) => ({
          ...session, 
    ...
    const fetchLocationData = async (sessionId, startTimeMillis, endTimeMillis) => {
      try {
        ...
        const formattedRoute = locationPoints
          ...
          console.log('lalala', formattedRoute)
