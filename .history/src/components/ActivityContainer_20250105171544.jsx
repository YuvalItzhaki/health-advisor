  ...
  useEffect(() => {
    const fetchRouteData = async () => {
      try {
        ...
        const sessionId = mostRecentSession[0].id;
    ...
    const fetchSessions = async (startTimeMillis, endTimeMillis) => {
      ...
      try {
        ...
        const sessions = data.session && Array.isArray(data.session) ? data.session : []; 
        return sessions;
      } catch (error) {
    ...
    const fetchLocationData = async (sessionId, startTimeMillis, endTimeMillis) => {
      try {
        const response = await axios.get(
          `http://localhost:5001/api/health/session-route?sessionId=${sessionId}&startTimeMillis=${startTimeMillis}&endTimeMillis=${endTimeMillis}`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
